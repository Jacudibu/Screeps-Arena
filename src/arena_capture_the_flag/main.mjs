...
import { Creep, StructureTower } from "game";
...
let myCreeps, myTowers, enemyCreeps, enemyFlag;
...
export function loop() {
  ...
  myTowers.forEach(tower => {
    let enemiesInRange = enemyCreeps.filter(enemy => getDistance(enemy, tower) < TOWER_RANGE);
    if (enemiesInRange.length > 0) {
      tower.attack(enemiesInRange[0]);
    }
    else {
      let damagedFriendsInRange = myCreeps.filter(i => getDistance(i, tower) < TOWER_RANGE && i.hits < i.hitsMax);
      if (damagedFriendsInRange.length > 0) {
        tower.heal(damagedFriendsInRange[0])
      }
    }
  });
}
...
function rangedAttacker(creep) {
  const targets = enemyCreeps.filter(i => getDistance(i, creep) <= 3)
    .sort((a,b) => getDistance(a, creep) - getDistance(b, creep));
